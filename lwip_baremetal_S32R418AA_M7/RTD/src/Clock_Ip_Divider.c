...
static void Clock_Ip_SetAuroraPllAtpdivDeDivOutput(Clock_Ip_DividerConfigType const* Config);
#endif
#ifdef CLOCK_IP_AURORA_PLL_ATP_PLL_PLL0DIV_DE_DIV_OUTPUT
static void Clock_Ip_SetAuroraPllAtpPlldivDeDivOutput(Clock_Ip_DividerConfigType const* Config);
#endif
    ...
    if (NULL_PTR != Config)
    {
        ...
        switch(DividerIndex)
        {
            case 0U:
                ...
                if (Config->Value != 0U)
                {
                    RegValue = Clock_Ip_apxAurora->PLLODIV_0;
                    RegValue |= ATP_PLLODIV_0_DE_MASK;
                    RegValue &= ~ATP_PLLODIV_0_DIV_MASK;
                    RegValue |= ATP_PLLODIV_0_DIV(Config->Value - 1U);
                    Clock_Ip_apxAurora->PLLODIV_0 = RegValue;
                }
                break;
            case 1U:
                /* Disable divider */
                Clock_Ip_apxAurora->PLLODIV_1 &= ~ATP_PLLODIV_1_DE_MASK;
                /* Check if divider is enabled */
                if (Config->Value != 0U)
                {
                    RegValue = Clock_Ip_apxAurora->PLLODIV_1;
                    RegValue |= ATP_PLLODIV_1_DE_MASK;
                    RegValue &= ~ATP_PLLODIV_1_DIV_MASK;
                    RegValue |= ATP_PLLODIV_1_DIV(Config->Value - 1U);
                    Clock_Ip_apxAurora->PLLODIV_1 = RegValue;
                }
                break;
            case 2U:
                /* Disable divider */
                Clock_Ip_apxAurora->PLLODIV_2 &= ~ATP_PLLODIV_2_DE_MASK;
                /* Check if divider is enabled */
                if (Config->Value != 0U)
                {
                    RegValue = Clock_Ip_apxAurora->PLLODIV_2;
                    RegValue |= ATP_PLLODIV_2_DE_MASK;
                    RegValue &= ~ATP_PLLODIV_2_DIV_MASK;
                    RegValue |= ATP_PLLODIV_2_DIV(Config->Value - 1U);
                    Clock_Ip_apxAurora->PLLODIV_2 = RegValue;
                }
                break;
            default:
                /* Unknown Divider Index*/
                break;
        }
    }
    else
    ...
    if (NULL_PTR != Config)
    {
        ...
        switch(DividerIndex)
        {
            case 0U:
                ...
                {
                    RegValue = Clock_Ip_apxAurora->PLLODIV_0;
                    RegValue |= ATP_PLL_PLLODIV_0_DE_MASK;
                    RegValue &= ~ATP_PLL_PLLODIV_0_DIV_MASK;
                    RegValue |= ATP_PLL_PLLODIV_0_DIV(Config->Value - 1U);
                    Clock_Ip_apxAurora->PLLODIV_0 = RegValue;
                }
                break;
            case 2U:
                /* Disable divider */
                Clock_Ip_apxAurora->PLLODIV_2 &= ~ATP_PLL_PLLODIV_2_DE_MASK;
                /* Check if divider is enabled */
                if (Config->Value != 0U)
                {
                    RegValue = Clock_Ip_apxAurora->PLLODIV_2;
                    RegValue |= ATP_PLL_PLLODIV_2_DE_MASK;
                    RegValue &= ~ATP_PLL_PLLODIV_2_DIV_MASK;
                    RegValue |= ATP_PLL_PLLODIV_2_DIV(Config->Value - 1U);
                    Clock_Ip_apxAurora->PLLODIV_2 = RegValue;
                }
                break;
            default:
...
#ifdef CLOCK_IP_AURORA_PLL_ATP_PLL0DIV_DE_DIV_OUTPUT
    {
        &Clock_Ip_SetAuroraPllAtpdivDeDivOutput,      /* Set */
    },
#endif
#ifdef CLOCK_IP_AURORA_PLL_ATP_PLL_PLL0DIV_DE_DIV_OUTPUT
    {
        &Clock_Ip_SetAuroraPllAtpPlldivDeDivOutput,      /* Set */
    },
#endif
