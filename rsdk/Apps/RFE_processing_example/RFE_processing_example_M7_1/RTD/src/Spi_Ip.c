...
static void Spi_Ip_CmdDmaTcdSGInit(uint8 Instance)
{
    ...
    DmaTcdList[1u].Value = (Spi_Ip_Uintptr)&Base->PUSHR.FIFO.CMD;    /* dest address write*/
...
static void Spi_Ip_TxDmaTcdSGInit(uint8 Instance)
{
    ...
    DmaTcdList[1u].Value = (Spi_Ip_Uintptr)&Base->PUSHR.FIFO.TX;    /* dest address write*/
...
static void Spi_Ip_TxDmaTcdSGConfig(uint8 Instance, uint8 TCDSGIndex, uint8 DisHwReq)
{
    ...
    {
        /* send default data */
        DmaTcdList[0u].Value = (Spi_Ip_Uintptr)&State->PhyUnitConfig->CmdDmaFast[TCDSGIndex].DefaultData; /* src address read */
    ...
    {
        DmaTcdList[0u].Value = (Spi_Ip_Uintptr)State->TxBuffer; /* src address read */
    }
    /* Update software TX DMA TCD Scatter Gather */
...
static void Spi_Ip_RxDmaTcdSGInit(uint8 Instance)
{
    ...
    DmaTcdList[0u].Value = (Spi_Ip_Uintptr)&Base->POPR; /* src address read */
    DmaTcdList[2u].Value = 0u;  /* no src offset */
...
static void Spi_Ip_RxDmaTcdSGConfig(uint8 Instance, uint8 TCDSGIndex, uint8 DisHwReq)
{
    ...
    {
        /* Discard data */
        DmaTcdList[0u].Value = (Spi_Ip_Uintptr)&Spi_Ip_u32DiscardData;    /* dest address write*/
        DmaTcdList[4u].Value = 0u; /* dest offset is 0 bytes */
    }
    else
    {
        DmaTcdList[0u].Value = (Spi_Ip_Uintptr)State->RxBuffer;    /* dest address write*/
    }
...
static void Spi_Ip_DmaConfig(uint8 Instance)
{
    ...
    /* Update last command */
    State->PushrCmdLast = State->PushrCmd & (~((uint16)((SPI_PUSHR_CONT_MASK | SPI_PUSHR_PP_MCSC_MASK) >> 16u)));
    ...
    #endif
    {
        ...
        DmaTcdList[1u].Value = (Spi_Ip_Uintptr)&State->PushrCmd;    /* default source address */
        if((boolean)TRUE == ClearCS)
        {
            if(1u == NumberCmdDmaIterWrite)
            {
                DmaTcdList[1u].Value = (Spi_Ip_Uintptr)&State->PushrCmdLast;    /* Clear CS command is source address */
        ...
        if((boolean)TRUE == EnScatterGather)
        {
            ...
            Spi_Ip_CmdDmaTcdSGConfig(Instance,
                                     ...
                                     (Spi_Ip_Uintptr)&State->PushrCmdLast, /* src address read */
    ...
    {
        /* send default data */
        DmaTcdList[0u].Value = (Spi_Ip_Uintptr)&State->ExternalDevice->DeviceParams->DefaultData; /* src address read */
        if(State->ExternalDevice->DeviceParams->FrameSize < 17u)
        {
    ...
    {
        /* Discard data */
        DmaTcdList[1u].Value = (Spi_Ip_Uintptr)&Spi_Ip_u32DiscardData;    /* dest address write*/
        DmaTcdList[6u].Value = 0u; /* dest offset is 0 bytes */
    }
    else
    {
        DmaTcdList[1u].Value = (Spi_Ip_Uintptr)State->RxBuffer;    /* dest address write*/
    }
...
)
{
    Spi_Ip_StateStructureType* State = Spi_Ip_apxStateStructureArray[Instance];
    SPI_Type* Base = Spi_Ip_apxBases[Instance];
    uint16 PushrCmd = State->PushrCmd;
    uint16 Mask;
    uint16 Index =  NumberOfWrites;

    if (Index > State->ExpectedCmdFifoWrites)
    {
        Index = State->ExpectedCmdFifoWrites;
    }
    while (Index > 1U)
    {
        Base->PUSHR.FIFO.CMD = PushrCmd;
        State->ExpectedCmdFifoWrites--;
        Index--;
    }
    if(1u == State->ExpectedCmdFifoWrites)
    {
        Mask = SPI_PUSHR_PP_MCSC_MASK >> 16u;
        if ((boolean)FALSE == State->KeepCs)
            Mask |= SPI_PUSHR_CONT_MASK >> 16u;
        PushrCmd &= ~Mask;
    }
    Base->PUSHR.FIFO.CMD = PushrCmd;
    State->ExpectedCmdFifoWrites--;
...
static void Spi_Ip_UpdateTxRxCounter(uint8 Instance, uint16 Length)
{
    ...
    /* FIFO word size is 16 bits. To write, the number of words to write is calculated */
    if (State->ExternalDevice->DeviceParams->FrameSize < 9u)
    {
    ...
    /* For read and cmd the number of frames is calculated */
                                     ...
                                     )
{
    ...
    else
    {
        ...
        if (State->CtareDtcpSupport)
        {
            ...
            Base->PUSHR.FIFO.CMD = (PushrCmd | (uint16)(SPI_PUSHR_CTAS(1U)>>16u)) & (~((uint16)((SPI_PUSHR_CONT_MASK | SPI_PUSHR_PP_MCSC_MASK) >> 16u)));
            /* Configure external device parameters  like: cs, clock phase, clock polarity. */
            #if (SPI_IP_DUAL_CLOCK_MODE == STD_ON)
        ...
        if((boolean)TRUE == ClearCS)
        {
            if(State->ExpectedCmdFifoWrites > 1u)
            {
                Spi_Ip_CmdDmaTcdSGConfig(   Instance,
                                            ...
                                            (Spi_Ip_Uintptr)&State->PhyUnitConfig->CmdDmaFast[Count].DmaFastPushrCmd,
            ...
            Spi_Ip_CmdDmaTcdSGConfig(   Instance,
                                        State->PhyUnitConfig->TxCmdDmaSGId[CmdTCDSGIndex],
                                        (Spi_Ip_Uintptr)&State->PhyUnitConfig->CmdDmaFast[Count].DmaFastPushrCmdLast,
