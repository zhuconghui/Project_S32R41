...
/*!
 * @brief Port_Ipw_SetOnlyInoutMode
 *
 * @details Port_Ipw_SetOnlyInoutMode function
 *
 *
 * @param[in] u8MscrSiulInstance Mscr instance
 * @param[in] u16MscrIdx Mscr ID
 ...
 * @return void
 **/
static inline void Port_Ipw_SetOnlyInoutMode
(
    uint8 u8MscrSiulInstance,
    uint16 u16MscrIdx
);
...
)
{
    ...
    for (u16PinIndex = (uint16)0U; u16PinIndex < u16NumUnusedPins; u16PinIndex++)
    {
        ...
        if (TRUE != pConfigPtr->pUnusedPads[u16PinIndex].bNoGpio)
        {
...
)
{
    switch(PinMode)
    {
        case PORT_ONLY_INOUT_MODE:
            Port_Ipw_SetOnlyInoutMode(u8MscrSiulInstance, u16MscrIdx);
            (void)PinIndex;
            (void)PinMode;
            (void)pConfigPtr;
            break;
