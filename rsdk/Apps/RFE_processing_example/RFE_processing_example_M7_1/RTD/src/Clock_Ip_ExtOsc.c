...
static void Clock_Ip_ResetFxoscCtrlOsconBypassStatusOscStat(Clock_Ip_XoscConfigType const* Config);
static void Clock_Ip_SetFxoscCtrlOsconBypassStatusOscStat(Clock_Ip_XoscConfigType const* Config);
static void Clock_Ip_CompleteFxoscCtrlOsconBypassStatusOscStat(Clock_Ip_XoscConfigType const* Config);
static void Clock_Ip_DisableFxoscCtrlOsconBypassStatusOscStat(Clock_Ip_NameType XoscName);
static void Clock_Ip_EnableFxoscCtrlOsconBypassStatusOscStat(Clock_Ip_XoscConfigType const* Config);
#endif
...
static void Clock_Ip_SetFxoscCtrlOsconBypassStatusOscStat(Clock_Ip_XoscConfigType const* Config)
{
    ...
    if (NULL_PTR != Config)
    {
        ...
        if (Config->Enable != 0U)
        {
            ...
            RegValue &= ~(FXOSC_CTRL_OSCON_MASK | FXOSC_CTRL_OSC_BYP_MASK);
            RegValue |=( FXOSC_CTRL_OSCON(1U) | FXOSC_CTRL_OSC_BYP(Config->BypassOption) );
...
static void Clock_Ip_CompleteFxoscCtrlOsconBypassStatusOscStat(Clock_Ip_XoscConfigType const* Config)
{
    ...
    if (NULL_PTR != Config)
    {
        ...
        if (Config->Enable != 0U)
        {
            ...
            do
            {
                FxoscStatus = ((Clock_Ip_apxXosc[Instance]->STAT & FXOSC_STATUS_OSC_STAT_MASK) >> FXOSC_STATUS_OSC_STAT_SHIFT);
...
static void Clock_Ip_DisableFxoscCtrlOsconBypassStatusOscStat(Clock_Ip_NameType XoscName)
{
    ...
    {
        /* Report timeout error */
        Clock_Ip_ReportClockErrors(CLOCK_IP_REPORT_TIMEOUT_ERROR, XoscName);
    }
}
static void Clock_Ip_EnableFxoscCtrlOsconBypassStatusOscStat(Clock_Ip_XoscConfigType const* Config)
{
...
#ifdef CLOCK_IP_FXOSC_CTRL_OSCON_BYP_STATUS_OSC_STAT
    {
        &Clock_Ip_ResetFxoscCtrlOsconBypassStatusOscStat,    /* Reset */
        &Clock_Ip_SetFxoscCtrlOsconBypassStatusOscStat,      /* Set */
        &Clock_Ip_CompleteFxoscCtrlOsconBypassStatusOscStat, /* Complete */
        &Clock_Ip_DisableFxoscCtrlOsconBypassStatusOscStat,  /* Disable */
        &Clock_Ip_EnableFxoscCtrlOsconBypassStatusOscStat,   /* Enable */
    },
#endif
#ifdef CLOCK_IP_FXOSC_OSCON_BYP_EOCV_GM_SEL
    {
