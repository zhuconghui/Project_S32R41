...
#include "rsdk_rfe_glue_spi_api.h"
...
==================================================================================================*/
static uint32_t map_spi_device(uint32_t glueLayerId, volatile struct SPI_tag **SPI_DEVICE);
/*==================================================================================================
 *                                       LOCAL FUNCTIONS
==================================================================================================*/
...
static uint32_t map_spi_device(uint32_t glueLayerId, volatile struct SPI_tag **SPI_DEVICE)
{
    ...
    {
        case 0:
        	*SPI_DEVICE = (volatile struct SPI_tag *)((volatile void *) &SPI_1);
            break;
        case 1:
            *SPI_DEVICE = (volatile struct SPI_tag *)((volatile void *) &SPI_2);
            break;
        default:
            
            result = SPI_GLUE_ERR_WRG_UNIT_ID;
    }
    return result;
}
...
uint32_t RsdkRfeGlueSpiInit(uint32_t glueLayerId)
{
    uint32_t result;
    volatile struct SPI_tag *SPI_DEVICE;
    
    result = map_spi_device(glueLayerId, &SPI_DEVICE);

    if (result == SPI_GLUE_SUCCESS)
    {
        // MCR initialization:
...
uint32_t RsdkRfeGlueSpiTransfer(uint32_t glueLayerId, uint8_t *pTxData, uint8_t *pRxData, uint16_t length)
{
    uint32_t result = SPI_GLUE_SUCCESS;
    uint32_t byteIdx = 0;
    ...
    if (length == 0)
    {
        result = SPI_GLUE_ERR_INVALID_LEN;
    }
