...
/**
* @file           rsdk_dsp_lal_matrix_upper_triangular_inv.c
*/
...
void Rsdk_LA_invUNs_Mof32p8Mif32p8(xb_vecN_2xf32 * pOutput,
		                           xb_vecN_2xf32 * pInput,
		                                  uint32_t size)
{
	xb_vecN_2xf32 (*pout)[size][size] = (xb_vecN_2xf32 (*)[size][size]) pOutput;
	xb_vecN_2xf32 sum;
	int32_t i, j, k;
	int32_t size_s = (int32_t)size;

	/* copying the input matrix in the inverse matrix */
	Rsdk_DT_copy_Mof32p8Mif32p8(pOutput, pInput, size, size);
	for (i = size_s - 1L; i >= 0L; i--)
	{
		/* reciprocal of the diagonal terms */
		(*pout)[i][i] = BBE_RECIPN_2XF32((*pout)[i][i]);
		for (j = i - 1L; j >= 0L; j--)
		{
			sum = (xb_vecN_2xf32)0L;
			for (k = i; k > j; k--)
			{
				sum = sum - ((*pout)[j][k] * (*pout)[k][i]);
			}
			(*pout)[j][i] = BBE_DIVN_2XF32(sum, (*pout)[j][j]);
		}
	}
};
...
void Rsdk_LA_invUNs_Mof32cp8Mif32cp8(xb_vecN_2xcf32 * pOutput,
		                             xb_vecN_2xcf32 * pInput,
		                                     uint32_t size)
{
	xb_vecN_2xcf32 (*pout)[size][size] = (xb_vecN_2xcf32 (*)[size][size]) pOutput;
	xb_vecN_2xcf32 sum;
	int32_t i, j, k;
	int32_t size_s = (int32_t)size;

	/* copying the input matrix in the inverse matrix */
	Rsdk_DT_copy_Mof32cp8Mif32cp8(pOutput, pInput, size, size);
	for (i = (size_s - 1L); i >= 0L; i--) {
		/* reciprocal of the diagonal terms */
		(*pout)[i][i] = BBE_RECIPN_2XCF32((*pout)[i][i]);
		for (j = (i - 1L); j >= 0L; j--)
		{
			...
			(*pout)[j][i] = BBE_DIVN_2XCF32(sum, (*pout)[j][j]);
		}
	}
};
...
void Rsdk_LA_invRectUNs_Mof32p8Mif32p8(xb_vecN_2xf32 * __restrict pOutput,
		                               xb_vecN_2xf32 * pInput,
											  ...
											  uint32_t cols)
{
	xb_vecN_2xf32 (*pout)[cols][rows] = (xb_vecN_2xf32 (*)[cols][rows]) pOutput;
	xb_vecN_2xf32 (*in)[rows][cols] = (xb_vecN_2xf32 (*)[rows][cols]) pInput;
    xb_vecN_2xf32 * pin1;
	xb_vecN_2xf32 * pin2;
	xb_vecN_2xf32 sum, op1, op2;
	int32_t i, j, k;
	/* use only the non-zero part of the input matrix */
	int32_t size_s = (int32_t)cols; 

	/* copy the input matrix in the inverse matrix, keeping the elements in the same position */
	for (i = 0L; i < size_s; i++) {
		for (j = i; j < size_s; j++)
		{
			(*pout)[i][j] = (*in)[i][j];
...
void Rsdk_LA_invRectUNs_Mof32cp8Mif32cp8(xb_vecN_2xcf32 * __restrict pOutput,
		                                 xb_vecN_2xcf32 * pInput,
												 ...
												 uint32_t cols)
{
	xb_vecN_2xcf32 (*pout)[cols][rows] = (xb_vecN_2xcf32 (*)[cols][rows]) pOutput;
