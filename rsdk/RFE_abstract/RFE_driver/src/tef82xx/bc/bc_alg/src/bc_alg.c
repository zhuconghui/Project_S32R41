...
{
    //IEEE 802.3 Cyclic Redundancy Check algoritm
    ...
    //No standard to check the range of Length and PreviousCRC
    if ((pData != NULL) && (pCRC != NULL))
    {
        *pCRC = ~PreviousCRC;
        //take in data byte by byte
        uint8_t *current = pData;
        //calc loop
#ifdef HOST_CPU_BIG_ENDIAN
        if((Len % 4) == 0)
        {
            while (Len > 0)
            {
                // CRC32 Table is in little-endian format, so CRC calculation is done by swaping the data. 
                for (int8_t Count = 3; Count >= 0; Count--)
                {
                    *pCRC = (*pCRC >> 8u) ^ CRC32_Table[(*pCRC & 0xFFu) ^ *(current + Count)];
                }
        ...
        while ((Len--) > 0)
        {
            *pCRC = (*pCRC >> 8u) ^ CRC32_Table[(*pCRC & 0xFFu) ^ *current];
            current++;
        }
#endif
